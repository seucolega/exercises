...
class MeltdownMitigationTest(unittest.TestCase):
    ...
    def test_is_criticality_balanced(self):
        ...
        test_data = ((750, 650, True), (799, 501, True), (500, 600, True),
                     ...
                     (800, 500.01, False),
                     (799.99, 500, False), (500.01, 999.99, False),
                     (625, 800, False),
                     (625.99, 800, False), (625.01, 799.99, False),
                     (799.99, 500.01, True),
                     (624.99, 799.99, True), (500, 1000, False),
                     (500.01, 1000, False),
                     (499.99, 1000, True))
    ...
    def test_reactor_efficiency(self):
        ...
        for variant, data in enumerate(test_data, start=1):
            ...
            with self.subTest(f'variation #{variant}', voltage=voltage,
                              current=current,
                              theoretical_max_power=theoretical_max_power,
                              expected=expected):
                # pylint: disable=assignment-from-no-return
                actual_result = reactor_efficiency(voltage, current,
