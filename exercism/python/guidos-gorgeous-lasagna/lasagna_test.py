...
class LasagnaTest(unittest.TestCase):
    ...
    def test_bake_time_remaining(self):
        ...
        for variant, (time, result) in enumerate(zip(input_data, result_data),
                                                 start=1):
    ...
    def test_preparation_time_in_minutes(self):
        ...
        for variant, (layers, time) in enumerate(zip(input_data, result_data),
                                                 start=1):
    ...
    def test_elapsed_time_in_minutes(self):
        ...
        result_data = [prep * 2 + elapsed for prep, elapsed in
                       zip(layer_data, time_data)]
        ...
        for variant, (layers, time, total_time) in enumerate(
            ...
            with self.subTest(f'variation #{variant}', layers=layers,
                ...
                self.assertEqual(elapsed_time_in_minutes(layers, time),
                                 total_time, msg=failure_msg)
