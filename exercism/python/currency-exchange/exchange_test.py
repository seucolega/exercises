...
class CurrencyExchangeTest(unittest.TestCase):
    ...
    def test_exchange_money(self):
        ...
        for variant, (input_data, output_data) in enumerate(
            ...
            with self.subTest(f"variation #{variant}", input_data=input_data,
                              output_data=output_data):
                self.assertEqual(
                    int(exchange_money(input_data[0], input_data[1])),
                    output_data)
    ...
    def test_get_change(self):
        ...
        for variant, (input_data, output_data) in enumerate(
            ...
            with self.subTest(f"variation #{variant}", input_data=input_data,
                              output_data=output_data):
                self.assertEqual(get_change(input_data[0], input_data[1]),
                                 output_data)
    ...
    def test_exchangeable_value(self):
        ...
        for variant, (inputs, output_data) in enumerate(
            ...
            with self.subTest(f"variation #{variant}", inputs=inputs,
                              output_data=output_data):
                self.assertEqual(
                    exchangeable_value(inputs[0], inputs[1], inputs[2],
                                       inputs[3]), output_data)
    ...
    def test_non_exchangeable_value(self):
        ...
        for variant, (inputs, output_data) in enumerate(
            ...
            with self.subTest(f"variation #{variant}", inputs=inputs,
                              output_data=output_data):
                self.assertEqual(
                    non_exchangeable_value(inputs[0], inputs[1], inputs[2],
                                           inputs[3]), output_data)
