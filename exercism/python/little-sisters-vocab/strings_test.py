...
class LittleSistersVocabTest(unittest.TestCase):
    ...
    def test_add_prefix_un(self):
        input_data = ['happy', 'manageable', 'fold', 'eaten', 'avoidable',
                      'usual']
        result_data = [f'un{item}' for item in input_data]
        number_of_variants = range(1, len(input_data) + 1)

        for variant, word, result in zip(number_of_variants, input_data,
                                         result_data):
            with self.subTest(f'variation #{variant}', word=word,
                              result=result):
    ...
    def test_make_word_groups_en(self):
        input_data = ['en', 'circle', 'fold', 'close', 'joy', 'lighten',
    ...
    def test_make_word_groups_pre(self):
        input_data = ['pre', 'serve', 'dispose', 'position', 'requisite',
                      'digest',
                      'natal', 'addressed', 'adolescent', 'assumption',
                      'mature', 'compute']
        result_data = (
    ...
    def test_make_word_groups_auto(self):
        input_data = ['auto', 'didactic', 'graph', 'mate', 'chrome', 'centric',
                      'complete',
    ...
    def test_make_words_groups_inter(self):
        input_data = ['inter', 'twine', 'connected', 'dependent', 'galactic',
                      'action',
                      'stellar', 'cellular', 'continental', 'axial',
                      'operative', 'disciplinary']
        result_data = (
    ...
    def test_remove_suffix_ness(self):
        input_data = ['heaviness', 'sadness', 'softness', 'crabbiness',
                      'lightness', 'artiness', 'edginess']
        result_data = ['heavy', 'sad', 'soft', 'crabby', 'light', 'arty',
                       'edgy']
        number_of_variants = range(1, len(input_data) + 1)
    ...
    def test_adjective_to_verb(self):
        ...
        for variant, sentence, index, result in zip(number_of_variants,
